...
## Plot here 
anim = ggplot(data = merged, 
       mapping = aes(x=date, y=Price)) +
  geom_line(color="#996515") +
  geom_point(color="white") +
  geom_text(aes(label=round(Price, 2)), size=2.5, color = "white", vjust=1.2, hjust=1.25) + 
  ...
  labs(title ="10-year US Treasury yield dips below 3%" , 
       subtitle = paste("Monthly yield, % (Jan 1972 - May 2022) " ,  "{format(frame_along, '%b-%Y')} ", sep = "                                                                           "), 
  ...
  transition_reveal(date) + view_follow() + enter_grow()

animate(anim, fps=5, height=500, width =700)
anim_save(filename = "ani2.gif")
